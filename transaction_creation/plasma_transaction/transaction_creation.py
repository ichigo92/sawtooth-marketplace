...
def create_agent(txn_key, batch_key, name, description):
    ...
    inputs = [addresser.make_agent_address(
        account_id=txn_key.get_public_key().as_hex())]

    outputs = [addresser.make_agent_address(
        account_id=txn_key.get_public_key().as_hex())]

    agent = payload_pb2.CreateAgent(
        name=name,
        description=description)
    payload = payload_pb2.TransactionPayload(
        payload_type=payload_pb2.TransactionPayload.CREATE_AGENT,
        create_agent=agent)
